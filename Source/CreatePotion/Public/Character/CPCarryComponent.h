...
private:
    FVector MakeReplacementDropLocation(const ACPThrowablePropBase* PreviousHeldProp, const ACPThrowablePropBase* NewProp) const;
...
private:
    ...
    // 교체 Drop 시 바닥에 박히는 것을 막기 위한 추가 높이
    UPROPERTY(EditAnywhere, Category = "Carry|Drop", meta = (ClampMin = "0.0", Units = "cm"))
    float ReplacementDropClearance = 2.f;
